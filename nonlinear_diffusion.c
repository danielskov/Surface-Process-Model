...
void d2_nonlin_diff(nodetype *nodes,model_parameters *parameters,int *indi)
{
  /*
  ...
  */	
  ...
  for (i=1;i<ny-1;i++)
      ...
      for (j=1;j<nx-1;j++)
	{
	  ...
	  //printf("node %d \n",node);
	  zx=(nodes[node+1].topsedi-nodes[node-1].topsedi)/(2*dx);
	  zy=(nodes[node+nx].topsedi-nodes[node-nx].topsedi)/(2*dy);
	  ...
	  zxy=(nodes[node+1+nx].topsedi+nodes[node-1-nx].topsedi-nodes[node-nx+1].topsedi-nodes[node+nx-1].topsedi)/(4*dx*dy);
				
	  norm=sqrt(zx*zx+zy*zy);
	  //if (node>50 & node<100)
	  // printf("node %d diff %g zxx %g zyy %g zx %g zy %g zxy %g  \n",node,diff,zxx,zyy,zx,zy,zxy);
      ...
      if (norm>0.5)
	{
	  printf("For stejlt %g venstre %ld \n",norm,i*nx+j);
	  printf("node %d diff %g zxx %g zyy %g zx %g zy %g zxy %g  \n",node,diff,zxx,zyy,zx,zy,zxy);
	  exit(1);
	}
      ...
      //right side
      node=i*nx+(nx-1);
      zx=(nodes[node].topsedi-nodes[node-1].topsedi)/(dx);
      zy=(nodes[node+nx].topsedi-nodes[node-nx].topsedi)/(2*dy);
      zxx=(nodes[node-1].topsedi+nodes[node-1].topsedi-2*nodes[node].topsedi)*inv_dx_square;
      zyy=(nodes[node+nx].topsedi+nodes[node-nx].topsedi-2*nodes[node].topsedi)*inv_dy_square;
      zxy=(nodes[node-1-nx].topsedi+nodes[node-1-nx].topsedi-nodes[node+nx-1].topsedi-nodes[node-1+nx].topsedi)/(4*dx*dy);
  ...
  for (j=1;j<nx-1;j++)
    {
      ...
      zxx=(nodes[node+1].topsedi+nodes[node-1].topsedi-2*nodes[node].topsedi)*inv_dx_square;
      zyy=(nodes[node-nx].topsedi+nodes[i*nx+j-nx].topsedi-2*nodes[node].topsedi)*inv_dy_square;
      zxy=(nodes[node-1-nx].topsedi+nodes[node-1-nx].topsedi-nodes[node-nx+1].topsedi-nodes[node-nx+1].topsedi)/(4*dx*dy);
  ...
  */
  ...
  if (norm>S_c)
    {
      printf("For stejlt %g øverste venstre \n",norm);
      printf("node %d diff %g zxx %g zyy %g zx %g zy %g zxy %g  \n",node,diff,zxx,zyy,zx,zy,zxy);
      exit(1);
    }
  ...
  if (norm>S_c)
    {
      printf("For stejlt %g øverste højre \n",norm);
      printf("node %d diff %g zxx %g zyy %g zx %g zy %g zxy %g  \n",node,diff,zxx,zyy,zx,zy,zxy);
      exit(1);
    }
